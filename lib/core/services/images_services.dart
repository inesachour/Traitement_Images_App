...
class ImagesService {
  ...
  Future<PGMImage> readPGM(String path) async {
    ...
    for (int i = 4; i < contents.length; i++) {
      ...
      List<int> pixels =
          contents[i].split(" ").map((e) => int.parse(e)).toList();
      img.mat.add(pixels);
    }
    return img;
  }
  ...
  PGMImage modifyContrastPGM(PGMImage img, Map<int, int> points, String path) {
    ...
    if (!points.keys.contains(img.maxValue)) {
      points.addAll({img.maxValue: img.maxValue});
    }
    ...
    for (int j = 0; j < sortedByKeyMap.length - 1; j++) {
      fn = newPoint(
          sortedByKeyMap.keys.elementAt(j),
          sortedByKeyMap.values.elementAt(j),
          sortedByKeyMap.keys.elementAt(j + 1),
          sortedByKeyMap.values.elementAt(j + 1));
      for (int i = sortedByKeyMap.keys.elementAt(j);
          i <= sortedByKeyMap.keys.elementAt(j + 1);
          i++) {
  ...
  PGMImage filtreConvolution(
      PGMImage img, List<List<int>> masque, String path) {
    PGMImage img2 = PGMImage.clone(img);
    img2.mat = img.mat.map((item) => item.map((e) => e).toList()).toList();
  ...
  PGMImage filtreHighBoost(PGMImage img, PGMImage imgFiltre, String path) {
    //TODO USER CHOOSE WHICH FILTER TO YA3MEL
    PGMImage img2 = PGMImage.clone(img);
    img2.mat = img.mat.map((item) => item.map((e) => e).toList()).toList();
    for (int row = 0; row < img.lx; row++) {
      for (int col = 0; col < img.ly; col++) {
  ...
  PPMImage seuillageManuel(
      PPMImage img, List<int> seuils, int option, String path) {
  ...
  int seuillageOtsuCouleur(List<int> pixels, int maxValue) {
    ...
    for (int i = 1; i < maxValue; i++) {
      ...
      variance =
          ((moy * probasCumule[i] - moyC1) * (moy * probasCumule[i] - moyC1)) /
              (probasCumule[i] * (1 - probasCumule[i]));
  ...
  List<int> erosionDilationColor(
      List<int> pixels, int x, int lx, int ly, int val) {
